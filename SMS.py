...
def forgot_password_screen():
    ...
    print("(demo thoi: reset mk ve default)\n")
    ...
    if ok == 0:
        ...
        print("\nreset xong roi nha, mk mac dinh la 123456")
    pause()
...
def ad_open_assign_class_sections():
    while True:
        ...
        if ch == "1":
            ...
            conn.commit()
            conn.close()
            print("\nTao lop xong, classSectionID =", class_id)
            pause()
    ...
    while True:
        ...
        if ch == "1":
            ...
            if datetime.now().date() > datetime.strptime(deadline, "%Y-%m-%d").date():
                print("\nhet han roi, ko cho sua diem nua")
                pause()
                continue
...
def student_dashboard(actor):
    while True:
        ...
        if ch == "1":
            ...
            print("\n(update contact info thoi nha, con MSSV vs major ko sua dc)")
